...
HRESULT LoadTextureDataFromFile(_In_z_ const wchar_t *fileName, std::unique_ptr<uint8_t[]> &ddsData,
				const DDS_HEADER **header, const uint8_t **bitData,
				size_t *bitSize) noexcept
{
	...
	*bitSize = 0;
	...
	ScopedHandle hFile(safe_handle(
		CreateFile2(fileName, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr)));
#else
	...
	// read the data in
	DWORD bytesRead = 0;
	if (!ReadFile(hFile.get(), ddsData.get(), fileInfo.EndOfFile.LowPart, &bytesRead,
		      nullptr)) {
		ddsData.reset();
		return HRESULT_FROM_WIN32(GetLastError());
	}
...
//--------------------------------------------------------------------------------------
HRESULT GetSurfaceInfo(_In_ size_t width, _In_ size_t height, _In_ DXGI_FORMAT fmt,
		       size_t *outNumBytes, _Out_opt_ size_t *outRowBytes,
		       _Out_opt_ size_t *outNumRows) noexcept
{
	...
	if (bc) {
		...
		rowBytes = ((uint64_t(width) + 1u) >> 1) * bpe;
		numBytes =
			(rowBytes * uint64_t(height)) + ((rowBytes * uint64_t(height) + 1u) >> 1);
...
//--------------------------------------------------------------------------------------
HRESULT CreateD3DResources(_In_ ID3D11Device *d3dDevice, _In_ uint32_t resDim, _In_ size_t width,
			   ...
			   _In_ bool isCubeMap,
			   _In_reads_opt_(mipCount *arraySize)
			   ...
			   _Outptr_opt_ ID3D11ShaderResourceView **textureView) noexcept
{
	...
	case D3D11_RESOURCE_DIMENSION_TEXTURE1D: {
		...
		desc.MiscFlags = miscFlags &
				 ~static_cast<unsigned int>(D3D11_RESOURCE_MISC_TEXTURECUBE);
			...
			if (textureView) {
				...
				if (arraySize > 1) {
					...
					SRVDesc.Texture1DArray.MipLevels =
						(!mipCount) ? UINT(-1) : desc.MipLevels;
					SRVDesc.Texture1DArray.ArraySize =
						static_cast<UINT>(arraySize);
				} else {
					SRVDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE1D;
					SRVDesc.Texture1D.MipLevels = (!mipCount) ? UINT(-1)
										  : desc.MipLevels;
				}
		...
		if (SUCCEEDED(hr) && tex) {
			if (textureView) {
				...
				if (isCubeMap) {
					...
					SRVDesc.Texture2D.MipLevels = (!mipCount) ? UINT(-1)
										  : desc.MipLevels;
				}
	...
	case D3D11_RESOURCE_DIMENSION_TEXTURE3D: {
		...
		if (SUCCEEDED(hr) && tex) {
			if (textureView) {
				...
				SRVDesc.Texture3D.MipLevels = (!mipCount) ? UINT(-1)
									  : desc.MipLevels;
			     ...
			     _Outptr_opt_ ID3D11ShaderResourceView **textureView) noexcept
{
	...
	bool autogen = false;
	if (mipCount == 1 && d3dContext &&
	    textureView) // Must have context and shader-view to auto generate mipmaps
	{
	...
	if (autogen) {
		...
		hr = CreateD3DResources(d3dDevice, resDim, width, height, depth, 0, arraySize,
					format, usage, bindFlags | D3D11_BIND_RENDER_TARGET,
					cpuAccessFlags,
		...
		if (SUCCEEDED(hr)) {
			hr = CreateD3DResources(d3dDevice, resDim, twidth, theight, tdepth,
						...
						bindFlags, cpuAccessFlags, miscFlags, loadFlags,
			...
			if (FAILED(hr) && !maxsize && (mipCount > 1)) {
				...
				case D3D_FEATURE_LEVEL_9_2:
					if (isCubeMap) {
						maxsize =
							512u /*D3D_FL9_1_REQ_TEXTURECUBE_DIMENSION*/;
					} else {
						maxsize =
							(resDim ==
				...
				hr = FillInitData(width, height, depth, mipCount, arraySize, format,
						  maxsize, bitSize, bitData, twidth, theight,
...
DDS_ALPHA_MODE GetAlphaMode(_In_ const DDS_HEADER *header) noexcept
{
	if (header->ddspf.flags & DDS_FOURCC) {
		if (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC) {
			auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10 *>(
				reinterpret_cast<const uint8_t *>(header) + sizeof(DDS_HEADER));
			auto const mode = static_cast<DDS_ALPHA_MODE>(
	...
	DDS_ALPHA_MODE *alphaMode) noexcept
{
	return CreateDDSTextureFromMemoryEx(d3dDevice, nullptr, ddsData, ddsDataSize, maxsize,
					    ...
					    DDS_LOADER_DEFAULT, texture, textureView, alphaMode);
}

_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemory(
	ID3D11Device *d3dDevice, ID3D11DeviceContext *d3dContext, const uint8_t *ddsData,
	size_t ddsDataSize, ID3D11Resource **texture, ID3D11ShaderResourceView **textureView,
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
	ID3D11Device *d3dDevice, const uint8_t *ddsData, size_t ddsDataSize, size_t maxsize,
	D3D11_USAGE usage, unsigned int bindFlags, unsigned int cpuAccessFlags,
	unsigned int miscFlags, DDS_LOADER_FLAGS loadFlags, ID3D11Resource **texture,
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromMemoryEx(
	ID3D11Device *d3dDevice, ID3D11DeviceContext *d3dContext, const uint8_t *ddsData,
	size_t ddsDataSize, size_t maxsize, D3D11_USAGE usage, unsigned int bindFlags,
	unsigned int cpuAccessFlags, unsigned int miscFlags, DDS_LOADER_FLAGS loadFlags,
	...
	DDS_ALPHA_MODE *alphaMode) noexcept
{
	...
	hr = CreateTextureFromDDS(d3dDevice, d3dContext, header, bitData, bitSize, maxsize, usage,
				  bindFlags, cpuAccessFlags, miscFlags, loadFlags, texture,
				  textureView);
	if (SUCCEEDED(hr)) {
		if (texture && *texture) {
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFile(
	...
	ID3D11ShaderResourceView **textureView, size_t maxsize, DDS_ALPHA_MODE *alphaMode) noexcept
{
...
_Use_decl_annotations_ HRESULT DirectX::CreateDDSTextureFromFileEx(
	...
	DDS_LOADER_FLAGS loadFlags, ID3D11Resource **texture,
	ID3D11ShaderResourceView **textureView, DDS_ALPHA_MODE *alphaMode) noexcept
{
	return CreateDDSTextureFromFileEx(d3dDevice, nullptr, fileName, maxsize, usage, bindFlags,
